...
@Component({
  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="bg-white rounded-2xl border border-stone-200 p-5 sm:p-6 shadow-xs">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-start sm:items-center gap-3">
            <div class="w-12 h-12 rounded-xl bg-teal-600 text-white flex items-center justify-center shrink-0 shadow-xs">
            ...
            <div>
              ...
              <p class="text-xs text-stone-500 mt-0.5">
                Patient: <span class="font-semibold text-stone-800">{{ patient().name }}</span> ({{ currentYear - patient().birthYear }}y) • Baseline: {{ chronicConditionsSummary() }}
              </p>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2 self-end sm:self-center">
            <div class="inline-flex rounded-xl bg-stone-100 p-1 border border-stone-200 text-xs font-medium">
              <button
                type="button"
                (click)="selectedRange.set('7d')"
                class="px-3 py-1 rounded-lg transition cursor-pointer"
                [class.bg-white]="selectedRange() === '7d'"
                [class.text-stone-900]="selectedRange() === '7d'"
                [class.shadow-xs]="selectedRange() === '7d'"
                [class.text-stone-500]="selectedRange() !== '7d'"
              >
                7 Days
              </button>
              <button
                ...
                [class.text-stone-900]="selectedRange() === '30d'"
                [class.shadow-xs]="selectedRange() === '30d'"
                [class.text-stone-500]="selectedRange() !== '30d'"
              >
                30 Days
              </button>
              <button
                ...
                [class.text-stone-900]="selectedRange() === 'all'"
                [class.shadow-xs]="selectedRange() === 'all'"
                [class.text-stone-500]="selectedRange() !== 'all'"
              >
                All
              </button>
            </div>
            ...
            <button
              ...
              class="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-stone-900 hover:bg-stone-800 active:scale-[0.98] disabled:opacity-50 text-white text-xs font-medium transition cursor-pointer shadow-xs"
        ...
        <div class="mt-4 pt-3 border-t border-stone-100 flex items-center gap-2 text-[11px] text-stone-600 overflow-x-auto no-scrollbar">
          <span class="font-semibold text-stone-700 shrink-0">Current Rx:</span>
          @for (med of patient().currentMedications; track med.name) {
            <span class="px-2 py-0.5 rounded-md bg-stone-50 border border-stone-200 shrink-0">
              {{ med.name }} {{ med.dosage }} ({{ med.frequency }})
            </span>
          }
          @if (patient().allergies.length > 0) {
            <span class="px-2 py-0.5 rounded-md bg-red-50 text-red-700 border border-red-200 shrink-0 font-medium">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        <div class="p-4 rounded-2xl bg-white border border-stone-200 shadow-xs">
          <div class="flex items-center justify-between text-stone-400 text-xs mb-1">
            <span>Entries Analyzed</span>
            <mat-icon class="text-base text-stone-400">receipt_long</mat-icon>
          </div>
          <p class="text-2xl font-bold text-stone-900">{{ filteredEntries().length }}</p>
          <p class="text-[10px] text-stone-500 mt-0.5">In chosen period</p>
        </div>
        ...
        <div class="p-4 rounded-2xl bg-white border border-stone-200 shadow-xs">
          ...
          <p class="text-2xl font-bold" [class.text-rose-600]="avgBp().isElevated" [class.text-stone-900]="!avgBp().isElevated">
            @if (avgBp().systolic && avgBp().diastolic) {
              {{ avgBp().systolic }}/{{ avgBp().diastolic }}
              <span class="text-xs font-normal text-stone-500">mmHg</span>
            } @else {
              <span class="text-stone-400 text-lg font-medium">N/A</span>
            }
          </p>
          <p class="text-[10px] mt-0.5" [class.text-rose-600]="avgBp().isElevated" [class.text-stone-500]="!avgBp().isElevated">
        ...
        <div class="p-4 rounded-2xl bg-white border border-stone-200 shadow-xs">
          ...
          <p class="text-2xl font-bold text-stone-900">
            @if (avgGlucose() !== null) {
              ...
              <span class="text-xs font-normal text-stone-500">mmol/L</span>
            } @else {
              <span class="text-stone-400 text-lg font-medium">N/A</span>
            }
          </p>
          <p class="text-[10px] text-stone-500 mt-0.5">Target: {{ patient().baselineVitals.fastingBloodSugarRange }}</p>
        </div>
        ...
        <div class="p-4 rounded-2xl bg-white border border-stone-200 shadow-xs">
          ...
          <p class="text-2xl font-bold" [class.text-emerald-600]="adherenceRate() >= 80" [class.text-amber-600]="adherenceRate() < 80">
            {{ adherenceRate() }}%
          </p>
          <p class="text-[10px] text-stone-500 mt-0.5">Prescription compliance</p>
        </div>
      </div>

      <!-- 3-Sentence AI Synthesis Card -->
      <div class="bg-linear-to-r from-teal-900 via-stone-900 to-stone-900 text-white rounded-2xl p-5 sm:p-6 shadow-md relative overflow-hidden">
        ...
        <div class="flex items-center justify-between gap-3 mb-3">
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs font-bold uppercase tracking-wider text-teal-300">Gemini 3.8 Clinical Synthesis</span>
          </div>
          @if (handover()) {
        ...
        <div class="text-stone-100 text-sm sm:text-base leading-relaxed">
          @if (isSynthesizing()) {
      ...
      <div class="bg-white rounded-2xl border border-stone-200 p-5 sm:p-6 shadow-xs">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center gap-2">
            <mat-icon class="text-teal-600 text-lg">show_chart</mat-icon>
            <h2 class="font-bold text-stone-900 text-sm sm:text-base">Longitudinal Vitals Trends</h2>
          </div>
          <div class="flex items-center gap-3 text-xs text-stone-500">
            ...
            <div class="flex items-center gap-1.5">
              <span class="w-3 h-0.5 bg-teal-500 inline-block rounded-full"></span>
        ...
        @if (vitalPoints().length > 0) {
          <div class="w-full overflow-x-auto">
            <div class="min-w-[500px] h-64 relative">
              <svg class="w-full h-full" viewBox="0 0 600 220" preserveAspectRatio="none">
                ...
                <rect x="50" y="55" width="530" height="35" fill="#f0fdf4" opacity="0.8" />
                <text x="55" y="77" fill="#15803d" font-size="9" font-family="sans-serif">Target BP Zone (120-130)</text>
                ...
                <!-- Systolic BP Line (Blue) -->
                @if (systolicPath()) {
                  <path [attr.d]="systolicPath()" fill="none" stroke="#2563eb" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
                }

                <!-- Diastolic BP Line (Teal) -->
                @if (diastolicPath()) {
                  <path [attr.d]="diastolicPath()" fill="none" stroke="#0d9488" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
                }

                <!-- Data Points and Tooltip Circles -->
                @for (pt of vitalPoints(); track pt.dateStr) {
                  @if (pt.systolic !== null) {
                    <circle [attr.cx]="pt.x" [attr.cy]="scaleYBp(pt.systolic)" r="4" fill="#2563eb" stroke="#ffffff" stroke-width="2" />
                    <text [attr.cx]="pt.x" [attr.y]="scaleYBp(pt.systolic) - 6" [attr.x]="pt.x" text-anchor="middle" fill="#1e40af" font-size="8" font-weight="bold">
                      {{ pt.systolic }}
                    </text>
                  }
                  @if (pt.diastolic !== null) {
                    <circle [attr.cx]="pt.x" [attr.cy]="scaleYBp(pt.diastolic)" r="3.5" fill="#0d9488" stroke="#ffffff" stroke-width="1.5" />
                  }
                  @if (pt.glucose !== null) {
          ...
          <div class="py-12 text-center text-stone-400 bg-stone-50 rounded-xl border border-dashed border-stone-200">
            <mat-icon class="text-3xl text-stone-300 mb-1">query_stats</mat-icon>
            <p class="text-xs font-medium text-stone-600">No vital sign readings logged during this time window</p>
            <p class="text-[11px] text-stone-400 mt-0.5">Entries with Blood Pressure or Glucose readings will plot automatically</p>
          </div>
        }
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-stone-200 p-5 sm:p-6 shadow-xs">
          <div class="flex items-center gap-2 mb-3">
            ...
            <h2 class="font-bold text-stone-900 text-sm sm:text-base">Recurring Symptom Patterns</h2>
          </div>
          <p class="text-xs text-stone-500 mb-4">Gemini-detected correlations across multiple journal observations.</p>
              ...
              @for (pattern of symptomPatterns(); track pattern.symptom) {
                <div class="p-3.5 rounded-xl bg-stone-50 border border-stone-200">
                  <div class="flex items-center justify-between mb-1.5">
                    <span class="font-semibold text-xs text-stone-900">{{ pattern.symptom }}</span>
                  ...
                  <p class="text-[11px] text-stone-600 leading-relaxed">
                    <span class="font-medium text-stone-700">Hypothesized Trigger:</span> {{ pattern.potentialTrigger }}
                  </p>
                </div>
              }
            </div>
          } @else {
            <div class="py-8 text-center text-stone-400 bg-stone-50 rounded-xl">
              <mat-icon class="text-2xl text-stone-300 mb-1">sentiment_satisfied</mat-icon>
              <p class="text-xs text-stone-600">No acute recurring symptom patterns detected</p>
              <p class="text-[10px] text-stone-400">Caregiver has reported stable baseline conditions.</p>
            </div>
          }
        </div>
        ...
        <div class="bg-white rounded-2xl border border-stone-200 p-5 sm:p-6 shadow-xs">
          <div class="flex items-center gap-2 mb-3">
            <mat-icon class="text-teal-600 text-lg">checklist</mat-icon>
            <h2 class="font-bold text-stone-900 text-sm sm:text-base">Suggested Clinical Discussion Agenda</h2>
          </div>
          <p class="text-xs text-stone-500 mb-4">High-priority discussion targets for the face-to-face consult.</p>

          <div class="space-y-2.5">
            @for (point of discussionPoints(); track point) {
              <div class="flex items-start gap-2.5 p-3 rounded-xl bg-teal-50/50 border border-teal-100">
                <mat-icon class="text-teal-700 text-base shrink-0 mt-0.5">check_box_outline_blank</mat-icon>
                <span class="text-xs text-stone-800 leading-relaxed">{{ point }}</span>
              </div>
            }
          </div>
        </div>
      ...
      <div class="bg-white rounded-2xl border border-stone-200 p-5 sm:p-6 shadow-xs">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center gap-2">
            <mat-icon class="text-stone-700 text-lg">history</mat-icon>
            <h2 class="font-bold text-stone-900 text-sm sm:text-base">Clinical Observations in Scope</h2>
          </div>
          <span class="text-xs text-stone-400 font-mono">{{ filteredEntries().length }} records</span>
        </div>

        @if (filteredEntries().length > 0) {
          <div class="space-y-4">
            @for (entry of filteredEntries(); track entry.id) {
              <div class="p-4 rounded-xl border border-stone-200 hover:border-stone-300 bg-stone-50/40 transition">
                <div class="flex items-center justify-between gap-2 mb-2">
                  <div class="flex items-center gap-2">
                    <span class="text-xs font-bold text-stone-800">{{ entry.createdAt | date:'mediumDate' }} at {{ entry.createdAt | date:'shortTime' }}</span>
                    @if (entry.safetyAlert) {
                      <span
                        ...
                        [class.text-red-800]="entry.safetyAlert.level === 'urgent'"
                  ...
                  <div class="flex items-center gap-1.5 text-[11px]">
                    @if (entry.extractedMetrics.bloodPressure; as bp) {
                      @if (bp.systolic && bp.diastolic) {
                        <span class="px-2 py-0.5 rounded-md bg-blue-50 text-blue-700 border border-blue-200 font-medium">
                          BP {{ bp.systolic }}/{{ bp.diastolic }}
                        </span>
                      }
                    }
                    @if (entry.extractedMetrics.bloodGlucose) {
                      <span class="px-2 py-0.5 rounded-md bg-amber-50 text-amber-800 border border-amber-200 font-medium">
                ...
                <div class="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-stone-600 bg-white p-3 rounded-lg border border-stone-200">
                  <div>
                    <span class="font-bold text-stone-700">Subjective:</span> {{ entry.clinicalSoap.subjective }}
                  </div>
                  <div>
                    <span class="font-bold text-stone-700">Objective:</span> {{ entry.clinicalSoap.objective }}
                  </div>
                  <div>
                    <span class="font-bold text-stone-700">Assessment:</span> {{ entry.clinicalSoap.assessment }}
                  </div>
                  <div>
                    <span class="font-bold text-stone-700">Plan:</span> {{ entry.clinicalSoap.plan }}
                  </div>
                </div>
              </div>
            }
